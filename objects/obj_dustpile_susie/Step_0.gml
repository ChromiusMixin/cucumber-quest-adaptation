...
if (con == 1 && __view_get(e__VW.XView, 0) >= 280 && obj_mainchara.y > 535)
{
    ...
    darksus.image_speed = 0.1;
    
    with (darksus)
        scr_depth();
    
    suspeed = point_distance(873, 602, 1116, 720) / 30;
...
if (con == 4 && __view_get(e__VW.XView, 0) >= 540 && obj_mainchara.y > 535)
{
    ...
    darksus2.image_speed = 0.1;
    
    with (darksus2)
        scr_depth();
    
    suspeed = point_distance(1116, 720, 1348, 643) / 30;
...
if (con == 27)
{
    thisremx = s.x;
    thisremy = s.y;
    s.speed = point_distance(s.x, s.y, partysus.x, partysus.y) / 20;
