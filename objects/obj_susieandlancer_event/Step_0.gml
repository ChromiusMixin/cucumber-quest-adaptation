if (con == 0 && global.interact == 0)
{
    ...
    if (obj_mainchara.x >= x)
    {
        ...
        if (global.flag[250] == 0)
        {
            ...
            scr_pan(2, 0, 140);
            mus_volume(global.currentsong[1], 0, 180);
...
if (con == 35)
{
    ...
    if (victory && s.visible == 0)
    {
        ...
        if (instance_exists(obj_susieenemy))
        {
            ...
            s.image_index = obj_susieenemy.siner / 12;
            s.image_speed = 0.083;
...
if (con == 48 && !d_ex())
{
    ...
    with (s)
    {
        scr_move_to_point_over_time(kx + 120, ky - 15, 120);
        image_speed = 0.1;
    }
...
if (con == 70 && !d_ex())
{
    ...
    con = 71;
    scr_pan_to_obj(obj_mainchara, 100);
    alarm[4] = 104;
}
