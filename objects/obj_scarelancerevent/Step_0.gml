...
if (con == 13 && d_ex() == 0)
{
    ...
    {
        hspeed = 1;
        vspeed = (((obj_scarelancerevent.y - y) + obj_scarelancerevent.sprite_height) - sprite_height) / 60;
        image_speed = 0.1;
    }
...
if (con == 31.1)
{
    for (i = 0; i < 3; i += 1)
    {
        ...
        with (cc[i])
        {
            scr_move_to_point_over_time(global.monstermakex[0] + 60, y, 60);
            depth = 50000;
        }
    }
...
if (con == 34 && d_ex() == 0)
{
    ...
    with (k)
        scr_move_to_point_over_time(obj_mainchara.x, obj_mainchara.y, 40);
    
    with (s)
        scr_move_to_point_over_time(global.cinstance[1].x, global.cinstance[1].y, 40);
    
    with (r)
        scr_move_to_point_over_time(global.cinstance[0].x, global.cinstance[0].y, 40);
...
if (con == 37 && d_ex() == 0)
{
    ...
    for (i = 0; i < 3; i += 1)
    {
        if (instance_exists(cc[i]))
        {
            ...
            with (cc[i])
                scr_move_to_point_over_time(global.monstermakex[i], global.monstermakey[i], 50);
        }
    }
