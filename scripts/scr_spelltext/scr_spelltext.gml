function scr_spelltext(arg0, arg1)
{
    ...
    switch (spell)
    {
        ...
        case 2.1:
            global.msg[0] = scr_84_get_subst_string("* ~1 cast COOLSPELL^1!&* ~1's ATTACK and MAGIC increased!/%", global.charname[global.char[caster]]);
            
            if (scr_checkstatus_hero(caster, "coolspell"))
                global.msg[0] = scr_84_get_subst_string("* ~1 cast COOLSPELL^1!&* The effect was refreshed!/%", global.charname[global.char[caster]]);
            
            break;
        
        case 2.2:
            scr_retarget_spell();
            var spellmsg = "* ~1 cast RUDESPELL!";
            
            if (global.monster[star] == 1)
            {
                if (!scr_checkstatus_monster(star, "def down"))
                    spellmsg += "&* ~2's DEFENSE lowered for 3 turns!";
                else
                    spellmsg += "&* Debuff turns refreshed!";
                
                if (global.monsterstatus[star] != 1)
                    spellmsg += " It became TIRED!";
            }
            
            spellmsg += "/%";
            global.msg[0] = scr_84_get_subst_string(spellmsg, global.charname[global.char[caster]], global.monstername[star]);
            break;
        
        case 2.3:
            global.msg[0] = scr_84_get_subst_string("* ~1 cast PEERPOISON^1!&* ~2 was POISONED!/%", global.charname[global.char[caster]], global.charname[global.char[star]]);
            
            if (scr_checkstatus_hero(star, "poison"))
                global.msg[0] = scr_84_get_subst_string("* ~1 cast PEERPOISON^1!&* ~2's POISON turns were refreshed!/%", global.charname[global.char[caster]], global.charname[global.char[star]]);
            
            break;
        
        case 2.4:
            global.msg[0] = scr_84_get_subst_string("* ~1 cast VENOSTRIKE^1!&* ~2 was POISONED!/%", global.charname[global.char[caster]], global.monstername[star]);
            
            if (scr_checkstatus_monster(star, "poison"))
                global.msg[0] = scr_84_get_subst_string("* ~1 cast VENOSTRIKE^1!&* ~2's POISON turns were refreshed!/%", global.charname[global.char[caster]], global.monstername[star]);
